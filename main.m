...
%     data_file_array = {'Test_for_Joszef_001-RNAv3_MES_Probe1.nirs'};
%     MCP_struct_S = create_MCP(data_file_array);


% For single subjectm single prob, and multiple runs
  fprintf('Double Run Testing:\n');
  data_file_array = {'Test_for_Joszef_001-RNAv3_MES_Probe1.nirs' 'Test_for_Joszef_002-BDZ_MES_Probe1.nirs'};
  subject_array_file{10} = [];
  for i = 1:10
      subject_array_file{i} = data_file_array;
  end
  
  MCP_for_subjects = MCP_for_multiple_subj(subject_array_file);
  
  time_window = 0:100;
  incl_channels = [3:5,13:19];
  incl_subjects = [1 3:6];

  MCPA_output = MCP_to_MCPA(MCP_for_subjects, subject_array_file, incl_subjects, incl_channels, time_window);
